...
function getRectangleString(width, height) {
    const top_of_rect = `┌${'─'.repeat(width - 2)}┐\n`,
        side_if_rect = `│${' '.repeat(width - 2)}│\n`,
        bottom_of_rect = `└${'─'.repeat(width - 2)}┘\n`;
    return top_of_rect + side_if_rect.repeat(height - 2) + bottom_of_rect;
}
